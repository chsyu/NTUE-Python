...
@router.get('/filter', response_model=List[HomeworkResponseSchema])
def get_homeworks_by_filter(filter: str = "", db: Session = Depends(get_db)):
    """ get homework by filter """
    return db_homework.get_homework_by_filter(filter, db)
