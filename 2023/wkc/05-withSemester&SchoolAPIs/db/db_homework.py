...
def get_all(db: Session):
    homework = str2List(db.query(DbHomework).all())
    if not homework:
...
def get_homework_by_semester(semester: str, db: Session):
    homework = str2List(db.query(DbHomework).filter(DbHomework.semester == semester).all())
    if not homework:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
...
def get_homework_by_school(school: str, db: Session):
    homework = str2List(db.query(DbHomework).filter(DbHomework.school == school).all())
    if not homework:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
