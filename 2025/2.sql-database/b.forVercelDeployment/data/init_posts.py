posts = [
    {
        "id": 1,
        "slug": "introduction-to-python",
        "title": "Python 入門指南",
        "author": "Alice",
        "content": "這篇文章介紹 Python 的基本語法、資料型態與常用函式，並提供簡單範例程式碼，幫助初學者快速入門，建立良好的開發基礎。",
        "likes": 42,
        "comments": [
            {"author": "Bob", "content": "很棒的入門教學！"},
            {"author": "Carol", "content": "程式碼範例很清楚"},
            {"author": "David", "content": "初學者必讀"}
        ]
    },
    {
        "id": 2,
        "slug": "advanced-python-tips",
        "title": "進階 Python 技巧",
        "author": "Bob",
        "content": "這篇文章包含裝飾器、生成器、協程、型別提示、錯誤處理等進階用法，讓你在開發時能寫出更簡潔、可讀性更高且效能更佳的 Python 程式。",
        "likes": 78,
        "comments": [
            {"author": "Alice", "content": "裝飾器部分講解得很詳細"},
            {"author": "Eva", "content": "協程的範例很實用"}
        ]
    },
    {
        "id": 3,
        "slug": "javascript-basics",
        "title": "JavaScript 基礎",
        "author": "Carol",
        "content": "本篇介紹瀏覽器端 JavaScript 的基本概念，包括變數宣告、事件監聽、DOM 操作與非同步程式設計，幫助讀者在前端開發打下穩固基礎。",
        "likes": 56,
        "comments": [
            {"author": "Frank", "content": "DOM 操作的說明很清楚"},
            {"author": "Grace", "content": "非同步程式設計部分有助理解"}
        ]
    },
    {
        "id": 4,
        "slug": "web-api-design",
        "title": "Web API 設計原則",
        "author": "David",
        "content": "分享 RESTful API 設計與版本管理策略，討論資源命名、狀態碼、認證與授權，並舉例如何規劃良好的資料回應格式，提高系統可維護性與擴充性。",
        "likes": 93,
        "comments": [
            {"author": "Henry", "content": "RESTful 設計原則整理得很好"},
            {"author": "Ivy", "content": "版本管理策略很實用"},
            {"author": "Jack", "content": "認證授權的部分可以再詳細一點"}
        ]
    },
    {
        "id": 5,
        "slug": "css-flexbox-layout",
        "title": "CSS Flexbox 佈局技巧",
        "author": "Eva",
        "content": "深入介紹 Flexbox 的核心概念與實用範例，包括主軸方向、對齊方式、彈性伸縮、排列順序與常見版型，讓你的前端排版更快速且彈性。",
        "likes": 67,
        "comments": [
            {"author": "Carol", "content": "Flexbox 範例很實用"},
            {"author": "Alice", "content": "對齊方式的說明幫助很大"}
        ]
    },
    {
        "id": 6,
        "slug": "react-hooks-guide",
        "title": "React Hooks 全攻略",
        "author": "Frank",
        "content": "如何使用 useState、useEffect 與自訂 Hooks 來管理狀態、生命週期及複用邏輯，並提供最佳實務建議，讓你的 React 應用更現代化與易於維護。",
        "likes": 89,
        "comments": [
            {"author": "Grace", "content": "自訂 Hooks 的部分很有啟發性"},
            {"author": "Bob", "content": "useState 範例很清楚"}
        ]
    },
    {
        "id": 7,
        "slug": "tailwindcss-styling",
        "title": "TailwindCSS 快速上手",
        "author": "Grace",
        "content": "用實例教你用 Tailwind 建立美觀介面，包含實用類別、版面配置、客製化主題與元件設計，顯著提升開發速度並保持一致的設計風格。",
        "likes": 72,
        "comments": [
            {"author": "Eva", "content": "客製化主題的部分很有用"},
            {"author": "David", "content": "實例很豐富"}
        ]
    },
    {
        "id": 8,
        "slug": "database-optimization",
        "title": "資料庫優化秘訣",
        "author": "Henry",
        "content": "探討索引設計、查詢快取、垂直與水平分割等最佳實務，幫助你在大型系統中保持高效能並減少瓶頸，提高資料庫穩定度與擴展性。",
        "likes": 105,
        "comments": [
            {"author": "Ivy", "content": "索引設計的建議很實用"},
            {"author": "Jack", "content": "查詢優化技巧幫助很大"},
            {"author": "Alice", "content": "分割策略很詳細"}
        ]
    },
    {
        "id": 9,
        "slug": "docker-deployment",
        "title": "Docker 部署入門",
        "author": "Ivy",
        "content": "學習如何將應用容器化並部署到雲端，涵蓋 Dockerfile、映像檔管理、網路設定與多容器協作，讓你的部署流程更一致、快速且可靠。",
        "likes": 84,
        "comments": [
            {"author": "Henry", "content": "Dockerfile 範例很清楚"},
            {"author": "Bob", "content": "多容器協作的部分很有用"}
        ]
    },
    {
        "id": 10,
        "slug": "fastapi-tutorial",
        "title": "FastAPI 實戰教學",
        "author": "Jack",
        "content": "一步步建構快速、高效能的 API 服務，講解路由、請求驗證、非同步處理、回傳格式及認證授權，幫助你掌握現代 Python 後端框架。",
        "likes": 96,
        "comments": [
            {"author": "David", "content": "FastAPI 教學很完整"},
            {"author": "Carol", "content": "非同步處理的說明很清楚"},
            {"author": "Frank", "content": "認證授權部分很實用"}
        ]
    }
]